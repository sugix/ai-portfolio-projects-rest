...
def naive_softmax_loss_and_gradient(
    ...
    # Calculating conditional prob distributions with softmax of two vector dot products

    pred = softmax(np.dot(outside_vectors, center_word_vec))

    # minimizing log likelihood loss
    loss = -np.log(pred[outside_word_idx])

    # Initialize grad vecs
    grad_outside_vecs = np.zeros(outside_vectors.shape)

    # Gold label one hot vector
    correct = np.zeros(pred.shape)
    correct[outside_word_idx] = 1

    grad_center_vec = np.dot(pred - correct, outside_vectors)
    grad_outside_vecs = np.outer(pred - correct, center_word_vec)
    ### END CODE HERE
...
def skipgram(
    ...
    ## Skip gram starts with center word and predicts the context/outer words

    centerWordIndex = word2ind[current_center_word]
    centerWordVector = center_word_vectors[centerWordIndex]

    # For context words + above center word, use neg_sampling_loss_and_gradient
    for context_word in outside_words:
        contextWordIndex = word2ind[context_word]
        cost, gcenter_vecs, goutside_vectors = word2vec_loss_and_gradient(
            centerWordVector, contextWordIndex, outside_vectors, dataset
        )

    # Accumulate loss, centerword grad and context word grad
        loss += cost
        grad_center_vecs[centerWordIndex] += gcenter_vecs
        grad_outside_vectors += goutside_vectors
