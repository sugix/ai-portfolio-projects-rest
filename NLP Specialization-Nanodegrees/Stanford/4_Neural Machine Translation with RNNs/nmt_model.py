...
class NMT(nn.Module):
    ...
    def encode(
        ...
        X_pack_padded_applied = pack_padded_sequence(input=X, lengths=source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_pack_padded_applied)
        enc_hiddens, batch_ind_sequence_len_list = pad_packed_sequence(
            sequence=enc_hiddens, batch_first=True
        )
    ...
    def beam_search(
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            hyp_word_ids = top_cand_hyp_pos - top_cand_hyp_pos.div(
                len(self.vocab.tgt), rounding_mode="floor"
            ) * len(
                self.vocab.tgt
            )  # top_cand_hyp_pos % len(self.vocab.tgt)
