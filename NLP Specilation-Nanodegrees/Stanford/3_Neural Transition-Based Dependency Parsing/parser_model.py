...
class ParserModel(nn.Module):
    ...
    def __init__(
        ...
        self.embed_to_hidden = nn.Linear(
            in_features=self.n_features * self.embed_size, out_features=self.hidden_size
        )
    ...
    def embedding_lookup(self, t):
        ...
        embedding_vectors = self.pretrained_embeddings(t)
        x = embedding_vectors.view(t.size(0), -1)

        assert x.shape == (t.shape[0], self.n_features*self.embed_size)
    ...
    def forward(self, t):
        ...
        hidden_layer_output = self.embed_to_hidden(embeddings)
        hidden_layer_activations = F.relu(hidden_layer_output)
        hidden_layer_activations_dropped = self.dropout(hidden_layer_activations)
        logits = self.hidden_to_logits(hidden_layer_activations_dropped)
        ### END CODE HERE
        return logits
