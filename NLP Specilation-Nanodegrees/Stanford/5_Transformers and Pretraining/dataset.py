...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        random_truncate_length = random.randint(4, int(self.block_size * 7 / 8))
        truncated_doc = document[:random_truncate_length]

        random_offset = random.randint(
            int(-0.125 * random_truncate_length), int(0.125 * random_truncate_length)
        )

        # As per the comment,1/4 the length of the truncated document on average.
        expected_mask_len = int(0.25 * random_truncate_length + random_offset)
        start_mask = random.randint(0, random_truncate_length - expected_mask_len)
        ...
        # [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] MASK_CHAR [pads]
        """
        [pads] is a string of repeated PAD_CHAR characters chosen so that the
    entire string is of length self.block_size.
    ...
    followed by the content that was removed, and the padding characters.
        ...
        encoded_input = torch.tensor([self.stoi[c] for c in input], dtype=torch.long)
        encoded_output = torch.tensor([self.stoi[c] for c in output], dtype=torch.long)
